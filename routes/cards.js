...
router.put('/:id', auth, async (req, res) => {
 ...
 router.get('/all', auth, async (req, res) => {
  ...
  const newCards = cards.map((card) => {
    favorite = favoriteCards.includes(card._id);
    return {favorite, ..._.pick(card, ["_id", "bizName", "bizDescription","bizAddress","bizPhone","bizImage","bizNumber","user_id"])};
  });
...
const cards = await Card.find({bizName: {$regex : req.query.input}});
  /* if (!cards) return res.status(404).send('The cards were not found.'); */
  res.send(cards);
  ...
  let card = new Card(
    {
      ...
      bizStarImage: req.body.bizStarImage ? req.body.bizStarImage : 'https://www.iconfinder.com/icons/430117/star_icon',
